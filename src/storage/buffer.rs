...
impl StorageBlockBuffer {
    pub fn open(&mut self) -> Result<(), ShmrError> {
        ...
        let interval = Duration::from_millis(SBB_FLUSH_INTERVAL);
...
mod tests {
    ...
    //
    //     let storage_block = StorageBlock::init_single(&io_engine);
    ...
    //         .write(
    //             &io_engine,
    //             data_to_write.len() + buffered_write_data1.len(),
    //             &buffered_write_data2,
    //         )
    //         .expect("Failed to write to the storage block buffer");
    //
    ...
    //         vec![
    //             0u8;
    //             data_to_write.len() + buffered_write_data1.len() + buffered_write_data2.len()
    //         ];
    //     let _ = storage_block_buffer
    ...
    //     assert_eq!(
    //         read_buffer2,
    //         vec![0u8, 1, 2, 3, 4, 5, 5, 6, 7, 8, 9, 10, 11]
    //     ); // Verifying the read data is as expected
    // }
}
