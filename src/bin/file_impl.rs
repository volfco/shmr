...
impl VirtualFile {
  ...
  pub fn read(&self, offset: u64, buf: &mut Vec<u8>) -> Result<usize> {
    ...
    for block_idx in block_range {
      ...
      trace!("reading block {} from offset {} to {}", block_idx, block_offset, block_end);
  ...
  pub fn write(&mut self, offset: u64, buf: &[u8]) -> Result<usize> {
    ...
    info!("writing {} bytes, starting at offset {}. target block range: {:?}", buf.len(), offset, &block_range);
