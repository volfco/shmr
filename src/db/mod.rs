...
impl FileDB {
    ...
    pub fn save_virtual_file(&self, vf: &VirtualFile) -> Result<(), ShmrError> {
        ...
        let _ = tx.execute("UPDATE inode SET size = ?2, ctime = ?3 WHERE inode = ?1", params![vf.ino, vf.size, now_unix()])?;

        // update the chunk map
        for (idx, chunk) in vf.chunk_map.iter().enumerate() {
            tx.execute("INSERT OR REPLACE INTO chunk (inode, chunk_idx, pointer) VALUES (?1, ?2, ?3)", params![vf.ino, idx, create_chunk_pointer(chunk.0, chunk.1)])?;
        }
...
#[cfg(test)]
mod tests {
    use crate::db::{create_chunk_pointer, parse_chunk_pointer};
